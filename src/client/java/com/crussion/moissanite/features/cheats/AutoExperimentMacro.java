...
public final class AutoExperimentMacro {
	...
	private static final String[] MAX_LEVEL_ENCHANT_TARGETS = {
			"nadeshiko dye",
			"prosecute vi",
			"protection vii",
			"sharpness vii",
	...
	private static int lastSeenGuiChangeTick;
	...
	private static final Map<Integer, SuperpairsCachedReward> superpairsCachedRewardsBySlot = new HashMap<>();
	private static final Map<String, Integer> superpairsPairClaimCounts = new HashMap<>();
	...
	private static int lastSuperpairsRevealSlot = -1;
	private static int pendingSuperpairsRevealSlot = -1;
	private static int pendingSuperpairsRevealTick;
	private static boolean pendingSuperpairsActivePairConfirm;
	private static int superpairsEffectivePairClicks;
	...
	private static void handleSolveSuperpairs(Minecraft client) {
		...
		if (resolvePendingSuperpairsReveal(menu)) {
			return;
		}
		...
		updateSuperpairsInstantFindState(menu);
		if (remainingClicks == 0) {
			handleSuperpairsNoClicksFallback(client);
			return;
		}

		boolean xpFallbackReady = isSuperpairsXpFallbackReady(menu);

		if (superpairsInstantFindPending && handleSuperpairsInstantFindTarget(client, menu)) {
			return;
		}
		...
		if (isNextSuperpairsClickStartingPair()
				&& xpFallbackReady) {
		...
		clickNextSuperpairsDiscovery(client, menu);
	}
	...
	private static void handleSuperpairsNoClicksFallback(Minecraft client) {
		clearActiveSuperpairsPair();
		superpairsInstantFindTargetSlot = -1;
		superpairsNoClicksFallbackClicks++;
		if (superpairsNoClicksFallbackClicks == 1) {
			sendMessage("Remaining clicks is 0, closing Superpairs board.");
		}
		if (superpairsNoClicksFallbackClicks >= WAIT_AFTER_CLICK_TICKS) {
			closeScreen(client);
			enterStep(MacroStep.WAIT_AFTER_SUPERPAIRS_CLAIM);
		}
	}

	private static void updateSuperpairsInstantFindState(ChestMenu menu) {
		if (menu == null) {
			return;
		}
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			if (superpairsHandledInstantFindSlots.contains(slotIndex)) {
				continue;
			}

			Slot slot = menu.slots.get(slotIndex);
			if (slot == null || !slot.hasItem() || !isSuperpairsInstantFindPowerup(slot.getItem())) {
				continue;
			}
			superpairsHandledInstantFindSlots.add(slotIndex);
			superpairsInstantFindPending = true;
			sendMessage("Instant Find active, selecting next target.");
			return;
		}
	}

	private static boolean handleSuperpairsInstantFindTarget(Minecraft client, ChestMenu menu) {
		...
		if (slot == -1) {
			slot = findNextSuperpairsRevealSlot(menu);
			target = "next hidden tile";
		}
		if (slot == -1) {
			slot = findAnySuperpairsClickableSlot(menu);
			target = "next available tile";
		}
		if (slot == -1) {
		...
		String clickedMatchKey = superpairsVisiblePairMatchKey(stackAt(menu, slot));
		if (!clickSuperpairsBoardSlot(client, menu, slot)) {
			return true;
		}

		superpairsInstantFindTargetSlot = clickedMatchKey.isBlank() ? -1 : slot;
		superpairsInstantFindPending = false;
		sendMessage("Instant Find clicked " + target + " at slot " + slot + ".");
		return true;
	}

	private static boolean resolvePendingSuperpairsReveal(ChestMenu menu) {
		if (pendingSuperpairsRevealSlot == -1) {
			return false;
		}
		if (menu == null || pendingSuperpairsRevealSlot < 0 || pendingSuperpairsRevealSlot >= menu.slots.size()) {
			pendingSuperpairsRevealSlot = -1;
			return false;
		}

		Slot slot = menu.slots.get(pendingSuperpairsRevealSlot);
		if (slot == null || !slot.hasItem()) {
			return waitForPendingSuperpairsReveal();
		}
	...
	private static boolean clickNextSuperpairsDiscovery(Minecraft client, ChestMenu menu) {
		if (!canClickSuperpairs()) {
			return true;
		}
		...
		if (revealSlot != -1) {
			return clickSuperpairsBoardSlot(client, menu, revealSlot);
		}

		int fillerSlot = findAnySuperpairsClickableSlot(menu);
		if (fillerSlot != -1) {
			return clickSuperpairsBoardSlot(client, menu, fillerSlot);
		}
		return false;
	}

	private static boolean clickSuperpairsBoardSlot(Minecraft client, ChestMenu menu, int slot) {
		if (isSuperpairsHiddenSlot(menu, slot)) {
			return clickSuperpairsRevealSlot(client, menu, slot);
		}
		return clickCountedVisibleSuperpairsSlot(client, menu, slot, false);
	}

	private static boolean clickSuperpairsRevealSlot(Minecraft client, ChestMenu menu, int slot) {
		if (!clickSuperpairsSlot(client, menu, slot)) {
			return false;
		}
		lastSuperpairsClickTick = macroElapsedTicks;
		lastSuperpairsBoardClickSlot = slot;
		superpairsClickedRevealSlots.add(slot);
		pendingSuperpairsRevealSlot = slot;
		pendingSuperpairsRevealTick = macroElapsedTicks;
		return true;
	}

	private static boolean clickKnownSuperpairsSlot(
			Minecraft client,
			ChestMenu menu,
			int slot,
		...
		if (isSuperpairsHiddenSlot(menu, slot)) {
			boolean clicked = clickSuperpairsRevealSlot(client, menu, slot);
			if (clicked) {
				pendingSuperpairsActivePairConfirm = activePairConfirm;
			}
			return clicked;
		}
		return clickCountedVisibleSuperpairsSlot(client, menu, slot, afterPairDelay);
	}

	private static boolean clickCountedVisibleSuperpairsSlot(
			Minecraft client,
			ChestMenu menu,
			int slotIndex,
			boolean afterPairDelay) {
		...
		ItemStack stack = slot.getItem();
		if (!clickSuperpairsSlot(client, menu, slotIndex)) {
			return false;
		}

		lastSuperpairsBoardClickSlot = slotIndex;
		lastSuperpairsClickTick = afterPairDelay
		...
		cacheVisibleSuperpairsReward(slotIndex, stack);
		applySuperpairsEffectiveClick(slotIndex, stack);
		return true;
	}

	private static void cacheVisibleSuperpairsRewards(ChestMenu menu) {
		if (menu == null) {
			return;
		}
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			cacheVisibleSuperpairsReward(slotIndex, slot.getItem());
		}
	}

	private static void cacheVisibleSuperpairsReward(int slotIndex, ItemStack stack) {
		if (slotIndex < 0 || stack == null || stack.isEmpty()
				|| isSuperpairsHiddenTile(stack)
				|| isSuperpairsNonPairPowerup(stack)) {
			return;
		}

		String matchKey = superpairsVisiblePairMatchKey(stack);
		if (matchKey.isBlank()) {
			return;
		}

		String priorityReward = priorityRewardSignature(stack);
		int xp = enchantingXpAmount(stack);
		String reward = priorityReward.isBlank() ? superpairsRewardName(stack) : priorityReward;
		boolean priority = !priorityReward.isBlank();
		superpairsCachedRewardsBySlot.put(
				slotIndex,
				new SuperpairsCachedReward(reward, matchKey, slotIndex, xp, priority));

		if (priority) {
			superpairsKnownHighTierSlots.add(slotIndex);
			superpairsFirstSlotsByEnchant.putIfAbsent(reward, slotIndex);
		}
	}

	private static void applySuperpairsEffectiveClick(int slot, ItemStack stack) {
		String matchKey = superpairsVisiblePairMatchKey(stack);
		if (matchKey.isBlank()) {
			return;
		}

		superpairsEffectivePairClicks++;
	...
	private static boolean isNextSuperpairsClickStartingPair() {
		return (superpairsEffectivePairClicks % 2) == 0;
	}

	private static boolean isWaitingForSuperpairsPairConfirmation() {
		return !isNextSuperpairsClickStartingPair()
				&& superpairsOpenPairSlot != -1
				&& !superpairsOpenPairMatchKey.isBlank();
	}

	private static SuperpairsPair findOpenSuperpairsPair(ChestMenu menu, boolean allowXpFallback) {
		if (!isWaitingForSuperpairsPairConfirmation()) {
			return null;
		}

		int mateSlot = findKnownSuperpairsPairMate(menu, superpairsOpenPairSlot, superpairsOpenPairMatchKey);
		if (mateSlot == -1) {
			return null;
		}
		if (isSuperpairsPairExhausted(superpairsOpenPairMatchKey, superpairsOpenPairSlot, mateSlot)) {
			return null;
		}

		SuperpairsCachedReward openReward = cachedSuperpairsReward(superpairsOpenPairSlot);
		ItemStack openStack = stackAt(menu, superpairsOpenPairSlot);
		boolean priority = openReward == null
				? !priorityRewardSignature(openStack).isBlank()
				: openReward.priority();
		int xp = openReward == null ? enchantingXpAmount(openStack) : openReward.xpValue();
		if (!priority && (!allowXpFallback || xp <= 0)) {
			return null;
		}
	...
	private static int firstAvailableSuperpairsPairSlot(ChestMenu menu, SuperpairsPair pair) {
		...
		if (pair.firstSlot() != lastSuperpairsBoardClickSlot
				&& slotMatchesSuperpairsMatchKey(menu, pair.firstSlot(), pair.matchKey())) {
			return pair.firstSlot();
		}
		if (pair.secondSlot() != lastSuperpairsBoardClickSlot
				&& slotMatchesSuperpairsMatchKey(menu, pair.secondSlot(), pair.matchKey())) {
			return pair.secondSlot();
		}
		return -1;
	}

	private static int activeSuperpairsConfirmSlot(ChestMenu menu, SuperpairsPair pair) {
		if (pair == null || !pair.matchKey().equals(superpairsOpenPairMatchKey)) {
			return -1;
		}
		if (superpairsOpenPairSlot == pair.firstSlot()
				&& slotMatchesSuperpairsMatchKey(menu, pair.secondSlot(), pair.matchKey())) {
			return pair.secondSlot();
		}
		if (superpairsOpenPairSlot == pair.secondSlot()
				&& slotMatchesSuperpairsMatchKey(menu, pair.firstSlot(), pair.matchKey())) {
			return pair.firstSlot();
		}
		return findKnownSuperpairsPairMate(menu, superpairsOpenPairSlot, pair.matchKey());
	}

	private static int findKnownSuperpairsPairMate(ChestMenu menu, int firstSlot, String matchKey) {
		if (menu == null || matchKey == null || matchKey.isBlank()) {
			return -1;
		}
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			if (slotIndex == firstSlot || slotIndex < 0 || slotIndex >= menu.slots.size()) {
				continue;
			}
			if (superpairsIgnoredPrioritySlots.contains(slotIndex)) {
				continue;
			}
			if (slotMatchesSuperpairsMatchKey(menu, slotIndex, matchKey)) {
				return slotIndex;
			}
		}
		return -1;
	...
	private static SuperpairsCachedReward cachedSuperpairsReward(int slotIndex) {
		return superpairsCachedRewardsBySlot.get(slotIndex);
	}

	private static boolean isCachedSuperpairsPrioritySlot(int slotIndex) {
		SuperpairsCachedReward cached = cachedSuperpairsReward(slotIndex);
		return cached != null && cached.priority();
	}

	private static ItemStack stackAt(ChestMenu menu, int slotIndex) {
		if (menu == null || slotIndex < 0 || slotIndex >= menu.slots.size()) {
	...
	private static void startSuperpairsPair(SuperpairsPair pair) {
		startSuperpairsPair(pair, false);
	}

	private static void startSuperpairsPair(SuperpairsPair pair, boolean firstClickAlreadyUsed) {
		int remainingClaims = remainingSuperpairsPairClaims(pair);
	...
	private static void startSuperpairsPair(SuperpairsPair pair, boolean firstClickAlreadyUsed, int pairClaims) {
		...
		if (firstClickAlreadyUsed) {
			...
			superpairsOpenPairMatchKey = pair.matchKey();
		}
		if (pair.xpValue() > 0) {
			sendMessage("Matching enchanting XP pair " + pair.reward() + " " + formatPairClaimCount(safePairClaims) + ".");
		} else {
			sendMessage("Matching priority reward " + pair.reward() + " " + formatPairClaimCount(safePairClaims) + ".");
		}
	}
	...
	private static boolean clickActiveSuperpairsPair(Minecraft client, ChestMenu menu) {
		...
		if (activeSuperpairsPairHasOpenSlot(activeSuperpairsPair)) {
			int confirmSlot = activeSuperpairsConfirmSlot(menu, activeSuperpairsPair);
			if (confirmSlot == -1 || confirmSlot == superpairsOpenPairSlot) {
				clearActiveSuperpairsPair();
				return false;
			}
		...
		if (isNextSuperpairsClickStartingPair()) {
			int startSlot = firstAvailableSuperpairsPairSlot(menu, activeSuperpairsPair);
			if (startSlot == -1) {
	...
	private static boolean activeSuperpairsPairHasOpenSlot(SuperpairsPair pair) {
		return pair != null
				&& superpairsOpenPairSlot != -1
				&& pair.matchKey().equals(superpairsOpenPairMatchKey)
				&& (superpairsOpenPairSlot == pair.firstSlot() || superpairsOpenPairSlot == pair.secondSlot());
	}
	...
	private static void finishActiveSuperpairsPair() {
		if (activeSuperpairsPair == null) {
			return;
		}
		markSuperpairsPairExhausted(activeSuperpairsPair);
	...
	private static void markSuperpairsPairExhausted(SuperpairsPair pair) {
		...
		superpairsExhaustedPairKeys.add(superpairsPairKey(pair.matchKey(), pair.firstSlot(), pair.secondSlot()));
		superpairsIgnoredPrioritySlots.add(pair.firstSlot());
		superpairsIgnoredPrioritySlots.add(pair.secondSlot());
	}

	private static SuperpairsPair findCachedSuperpairsPriorityPair(ChestMenu menu) {
		Map<String, SuperpairsCachedReward> firstByMatchKey = new HashMap<>();
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			SuperpairsCachedReward cached = cachedSuperpairsReward(slotIndex);
			...
			SuperpairsCachedReward first = firstByMatchKey.get(cached.matchKey());
			if (first == null) {
				firstByMatchKey.put(cached.matchKey(), cached);
				continue;
			}
			if (isSuperpairsPairExhausted(cached.matchKey(), first.slot(), cached.slot())) {
				continue;
			}
			return new SuperpairsPair(cached.reward(), cached.matchKey(), first.slot(), cached.slot(), cached.xpValue());
		}
		return null;
	}
	...
	private static SuperpairsPair scanSuperpairsBoard(ChestMenu menu) {
		...
		SuperpairsPair cachedPair = findCachedSuperpairsPriorityPair(menu);
		if (cachedPair != null) {
			return cachedPair;
		}
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			if (firstSlot != slotIndex) {
				...
				superpairsKnownHighTierSlots.add(slotIndex);
				String matchKey = priorityRewardMatchKey(reward);
				if (isSuperpairsPairExhausted(matchKey, firstSlot, slotIndex)) {
					continue;
				}
				return new SuperpairsPair(reward, matchKey, firstSlot, slotIndex, 0);
			}
		}
	...
	private static SuperpairsPair findLastRevealedPriorityPair(ChestMenu menu) {
		if (menu == null || lastSuperpairsRevealSlot < 0 || lastSuperpairsRevealSlot >= menu.slots.size()) {
			return null;
		}
		if (superpairsIgnoredPrioritySlots.contains(lastSuperpairsRevealSlot)) {
			return null;
		}
		SuperpairsCachedReward cached = cachedSuperpairsReward(lastSuperpairsRevealSlot);
		if (cached == null || !cached.priority()) {
			return null;
		}

		int mateSlot = findKnownSuperpairsPairMate(menu, lastSuperpairsRevealSlot, cached.matchKey());
		if (mateSlot != -1 && !isSuperpairsPairExhausted(cached.matchKey(), lastSuperpairsRevealSlot, mateSlot)) {
			return new SuperpairsPair(cached.reward(), cached.matchKey(), mateSlot, lastSuperpairsRevealSlot, cached.xpValue());
		}
		return null;
	}

	private static SuperpairsPair findLastRevealedEnchantingXpPair(ChestMenu menu) {
		if (menu == null || lastSuperpairsRevealSlot < 0 || lastSuperpairsRevealSlot >= menu.slots.size()) {
			return null;
		}
		if (superpairsIgnoredPrioritySlots.contains(lastSuperpairsRevealSlot)) {
		...
		SuperpairsCachedReward cached = cachedSuperpairsReward(lastSuperpairsRevealSlot);
		if (cached == null || cached.xpValue() <= 0 || cached.matchKey().isBlank()) {
			return null;
		}
	...
	private static boolean isSuperpairsPairExhausted(String matchKey, int firstSlot, int secondSlot) {
		String key = superpairsPairKey(matchKey, firstSlot, secondSlot);
		return superpairsExhaustedPairKeys.contains(key)
	...
	private static int remainingSuperpairsPairClaims(SuperpairsPair pair) {
		...
		int completedClaims = superpairsPairClaimCounts.getOrDefault(superpairsPairKey(pair), 0);
		return Math.max(0, configuredSuperpairsPairClaims() - completedClaims);
	}

	private static int recordSuperpairsPairClaim(SuperpairsPair pair) {
		if (pair == null) {
			return 0;
		}
		String key = superpairsPairKey(pair);
		int completedClaims = superpairsPairClaimCounts.getOrDefault(key, 0) + 1;
		superpairsPairClaimCounts.put(key, completedClaims);
		return completedClaims;
	}

	private static String superpairsPairKey(SuperpairsPair pair) {
		if (pair == null) {
			return "";
		}
		return superpairsPairKey(pair.matchKey(), pair.firstSlot(), pair.secondSlot());
	}

	private static String superpairsPairKey(String matchKey, int firstSlot, int secondSlot) {
		int lowSlot = Math.min(firstSlot, secondSlot);
		int highSlot = Math.max(firstSlot, secondSlot);
		return matchKey + ":" + lowSlot + ":" + highSlot;
	}
	...
	private static SuperpairsPair findHighestEnchantingXpPair(ChestMenu menu) {
		...
		Map<String, List<Integer>> slotsByMatchKey = new HashMap<>();
		Map<String, Integer> xpByMatchKey = new HashMap<>();
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			String matchKey = cached.matchKey();
			if (matchKey.isBlank()) {
				continue;
			}
			slotsByMatchKey.computeIfAbsent(matchKey, ignored -> new java.util.ArrayList<>()).add(slotIndex);
			xpByMatchKey.put(matchKey, xp);
		}

		for (Map.Entry<String, List<Integer>> entry : slotsByMatchKey.entrySet()) {
			...
			int xp = xpByMatchKey.getOrDefault(matchKey, 0);
			if (xp <= 0 || (bestPair != null && xp < bestPair.xpValue())) {
				continue;
			}
			for (int firstIndex = 0; firstIndex < slots.size(); firstIndex++) {
				for (int secondIndex = firstIndex + 1; secondIndex < slots.size(); secondIndex++) {
					int firstSlot = slots.get(firstIndex);
					int secondSlot = slots.get(secondIndex);
					if (isSuperpairsPairExhausted(matchKey, firstSlot, secondSlot)) {
						continue;
					}
					if (bestPair == null || xp > bestPair.xpValue()) {
						bestPair = new SuperpairsPair(formatXpReward(xp), matchKey, firstSlot, secondSlot, xp);
					}
					firstIndex = slots.size();
					break;
				}
			}
		}
	...
	private static boolean isSuperpairsXpFallbackReady(ChestMenu menu) {
		return isSuperpairsMostlyUncovered(menu) && !hasKnownSuperpairsPriorityReward(menu);
	}
	...
	private static boolean isSuperpairsMostlyUncovered(ChestMenu menu) {
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			if ((slot != null && slot.hasItem() && !isSuperpairsHiddenTile(slot.getItem()))
					|| superpairsCachedRewardsBySlot.containsKey(slotIndex)
					|| superpairsClickedRevealSlots.contains(slotIndex)) {
				uncoveredSlots++;
			}
		}
	...
	private static int findNextSuperpairsRevealSlot(ChestMenu menu) {
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			if (superpairsCachedRewardsBySlot.containsKey(slotIndex)) {
				continue;
			}
		...
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			...
			if (slot != null
					&& slot.hasItem()
					&& isSuperpairsHiddenTile(slot.getItem())
					&& !superpairsIgnoredPrioritySlots.contains(slotIndex)
					&& !isCachedSuperpairsPrioritySlot(slotIndex)) {
				return slotIndex;
			}
		}
		return -1;
	...
	private static int findRandomSuperpairsClickableSlot(ChestMenu menu) {
		if (menu == null) {
			return -1;
		}
		...
		int selected = java.util.concurrent.ThreadLocalRandom.current().nextInt(candidates);
		for (int slotIndex : SUPERPAIRS_BOARD_SLOTS) {
			if (!isSuperpairsClickableSlot(menu, slotIndex)) {
				continue;
			}
	...
	private static boolean isSuperpairsClickableSlot(ChestMenu menu, int slotIndex) {
		if (menu == null || slotIndex < 0 || slotIndex >= menu.slots.size()) {
			return false;
		}
		Slot slot = menu.slots.get(slotIndex);
	...
	private static boolean isMainMenu(Minecraft client) {
		return currentTitle(client).contains(MAIN_TITLE);
	}
	...
	private static void trackGuiForClickDelay(Minecraft client) {
		...
		if (menu == null || title.isBlank()) {
			...
			lastSeenGuiTitle = "";
			lastSeenGuiChangeTick = macroElapsedTicks;
			return;
		}
	...
	private static boolean isSuperpairsHiddenSlot(ChestMenu menu, int slotIndex) {
		if (menu == null || slotIndex < 0 || slotIndex >= menu.slots.size()) {
			return false;
		}
		Slot slot = menu.slots.get(slotIndex);
		return slot != null && slot.hasItem() && isSuperpairsHiddenTile(slot.getItem());
	}

	private static boolean isSuperpairsInstantFindPowerup(ItemStack stack) {
		if (stack == null || stack.isEmpty()) {
			return false;
		}
		String name = normalizedStackName(stack);
		String text = normalizedStackText(stack);
		return name.contains("instant find")
				|| (text.contains("instant find") && text.contains("powerup for next click"));
	}

	private static boolean isSuperpairsNonPairPowerup(ItemStack stack) {
		if (stack == null || stack.isEmpty()) {
			return false;
		}
		String text = normalizedStackName(stack) + " " + normalizedStackText(stack);
		return text.contains("powerup")
				|| text.contains("+3 clicks")
				|| text.contains("3 clicks")
				|| text.contains("half xp refund")
				|| text.contains("full xp refund")
				|| text.contains("instant find");
	}

	private static String superpairsVisiblePairMatchKey(ItemStack stack) {
		if (stack == null || stack.isEmpty() || isSuperpairsHiddenTile(stack) || isSuperpairsNonPairPowerup(stack)) {
			return "";
		}

		String priority = priorityRewardSignature(stack);
		if (!priority.isBlank()) {
			return priorityRewardMatchKey(priority);
		}

		String xpKey = enchantingXpRewardMatchKey(stack);
		if (!xpKey.isBlank()) {
			return xpKey;
		}
		...
		return "item:" + itemPath(stack) + ":" + name;
	}

	private static String superpairsRewardName(ItemStack stack) {
		if (stack == null || stack.isEmpty()) {
			return "reward";
		}
		String priority = priorityRewardSignature(stack);
		...
		int xp = enchantingXpAmount(stack);
		if (xp > 0) {
			return formatXpReward(xp);
		}
		String name = normalizedStackName(stack);
		return name.isBlank() ? "reward" : name;
	}
	...
	private static String priorityRewardSignature(ItemStack stack) {
		...
		String extraTarget = writtenTargetSignature(stack, SUPERPAIRS_EXTRA_CLAIM_TARGETS);
		if (!extraTarget.isBlank()) {
			return extraTarget;
		}
	...
	private static String priorityRewardMatchKey(String reward) {
		if (reward == null || reward.isBlank()) {
			return "";
		}
		return "priority:" + TextNormalizer.normalize(reward);
	}
	...
	private static String highTierEnchantSignature(ItemStack stack) {
		...
		return writtenTargetSignature(stack, MAX_LEVEL_ENCHANT_TARGETS);
	}
	...
	private static String enchantingXpRewardMatchKey(ItemStack stack) {
		int xp = enchantingXpAmount(stack);
		if (xp <= 0) {
			return "";
		}
		return "xp:" + xp + ":" + itemPath(stack) + ":" + normalizedStackName(stack);
	}

	private static int readSuperpairsRemainingClicks(ChestMenu menu) {
		if (menu == null) {
			return -1;
		}
		...
		for (int slotIndex = 0; slotIndex < maxSlot; slotIndex++) {
			...
			int remainingClicks = parseRemainingClicks(normalizedStackText(slot.getItem()));
			if (remainingClicks >= 0) {
	...
	private static int parseRemainingClicks(String text) {
		if (text == null || text.isBlank()) {
			return -1;
		}
		int marker = text.indexOf("remaining clicks");
		if (marker == -1) {
			return -1;
		}

		int cursor = marker + "remaining clicks".length();
		int scanEnd = Math.min(text.length(), cursor + SUPERPAIRS_REMAINING_CLICKS_SCAN_CHARS);
		while (cursor < scanEnd && !Character.isDigit(text.charAt(cursor))) {
			cursor++;
		}
		if (cursor >= scanEnd) {
			return -1;
	...
	private static String writtenTargetSignature(ItemStack stack, String[] targets) {
		if (stack == null || stack.isEmpty() || targets == null) {
			return "";
		}

		String bestTarget = "";
		for (String target : targets) {
			if (stackContainsWrittenTarget(stack, target)
					&& TextNormalizer.normalize(target).length() > TextNormalizer.normalize(bestTarget).length()) {
				bestTarget = target;
			}
		}
		return bestTarget;
	...
	private static boolean stackContainsWrittenTarget(ItemStack stack, String target) {
		if (target == null || target.isBlank()) {
			return false;
		}

		String normalizedTarget = TextNormalizer.normalize(target);
		if (matchesWrittenTargetLine(stack.getHoverName().getString(), normalizedTarget)
				|| matchesWrittenTargetLine(stack.getDisplayName().getString(), normalizedTarget)) {
			return true;
		}
		...
		for (net.minecraft.network.chat.Component line : lore.lines()) {
			if (matchesWrittenTargetLine(line.getString(), normalizedTarget)) {
				return true;
			}
		}
		return false;
	...
	private static boolean matchesWrittenTargetLine(String line, String normalizedTarget) {
		if (line == null || normalizedTarget == null || normalizedTarget.isBlank()) {
			return false;
		}
		return normalizeRewardLine(line).equals(normalizedTarget);
	}

	private static String normalizeRewardLine(String line) {
		String normalized = TextNormalizer.normalize(line);
		int start = 0;
		while (start < normalized.length() && !Character.isLetterOrDigit(normalized.charAt(start))) {
			start++;
		}
	...
	private static boolean clickSlot(Minecraft client, ChestMenu menu, int slot) {
		...
		int clickDelayMs = configuredUiClickDelayMs();
		if (!GuiClickThrottle.clickSlotLikeUser(client, menu, slot, clickDelayMs, clickDelayMs)) {
			return false;
		}
	...
	private static boolean canClickUiSlot() {
		int delayTicks = configuredUiClickDelayTicks();
		return macroElapsedTicks - lastSeenGuiChangeTick >= delayTicks
				&& macroElapsedTicks - lastUiClickTick >= delayTicks;
	}
	...
	private static int configuredUiClickDelayMs() {
		Integer value = UiDefinitions.AUTO_EXPERIMENTS_MACRO_UI_CLICK_DELAY.get();
		int configured = value == null ? DEFAULT_UI_CLICK_DELAY_MS : value;
		return Mth.clamp(configured, MIN_UI_CLICK_DELAY_MS, MAX_UI_CLICK_DELAY_MS);
	}

	private static int configuredUiClickDelayTicks() {
		return ticksForDelayMs(configuredUiClickDelayMs());
	}
	...
	private static void resetSuperpairsState() {
		...
		superpairsCachedRewardsBySlot.clear();
		superpairsPairClaimCounts.clear();
	...
	private record SuperpairsPair(String reward, String matchKey, int firstSlot, int secondSlot, int xpValue) {
	}

	private record SuperpairsCachedReward(String reward, String matchKey, int slot, int xpValue, boolean priority) {
	}
